...
# Get Person and all its subclasses recursively
person_classes = []

def get_subclasses(x):
    if x not in person_classes:
        person_classes.append(x)
...
WHERE {
    ...
    {
        ?person p:hasColleague ?c1 .
        ?c1 p:ownsPet ?pet .
        ?pet rdf:type p:Animal .
    }
    UNION
    # collegue who has a colleague who has a dog
    {
        ?person p:hasColleague ?c1 .
