...
Original file is located at
    https://colab.research.google.com/drive/1llzoitL1_PJtiMZxZjQry9ECWiC0w6q4
...
We create the URI's first, using Namespace trying to make it easier each time we need to use this URI.
"""

ontology = Namespace("http://oeg.fi.upm.es/def/people#")
person = Namespace("http://oeg.fi.upm.es/resource/person/")

"""Now, we save them in the graph:"""
...
# Definimos la jerarquía de clases y valores:
...
g.add((ontology.hasName, RDF.type, RDF.Property)) # a que tipo de recurso y el tipo: propiedad
g.add((ontology.hasName, RDFS.domain, ontology.Person)) # a que clase se le aplica
g.add((ontology.hasName, RDFS.range, RDFS.Literal)) # a que tipo de valor apunta
g.add((ontology.hasName, RDFS.label, Literal("hasName", datatype=XSD.string)))

g.add((ontology.hasColleague, RDF.type, RDF.Property)) # a que tipo de recurso y el tipo: propiedad
g.add((ontology.hasColleague, RDFS.domain, ontology.Person)) # a que clase se le aplica
g.add((ontology.hasColleague, RDFS.range, ontology.Person)) # a que tipo de valor apunta
g.add((ontology.hasColleague, RDFS.label, Literal("hasColleague", datatype=XSD.string)))

g.add((ontology.hasHomePage, RDF.type, RDF.Property)) # a que tipo de recurso y el tipo: propiedad
g.add((ontology.hasHomePage, RDFS.domain, ontology.FullProfessor)) # a que clase se le aplica
g.add((ontology.hasHomePage, RDFS.range, RDFS.Literal)) # a que tipo de valor apunta
